...
boto_ses = boto3.Session(region_name=aws_region)
...
def process(
    ...
    """
    # create s3 object for debug
    s3_client.put_object(
        Bucket=f"{aws_account_id}-{aws_region}-data",
        Key=(
            "projects/aws-lambda-backed-custom-resources-poc/01-Example-Update-Custom-Resource/requests/"
            f"{str(datetime.now())}.json"
        ),
        Body=json.dumps(request.to_dict(), indent=4),
    )
    data = {"name": f"CustomResourcePocIamGroup{random.randint(100, 999)}"}
    print("---------- Response data ----------")
    data_body = json.dumps(data)
    print(f"{data_body = }")

    if request.is_create_type():
        physical_resource_id = "the-initial-physical-resource-id"
    elif request.is_update_type():
        physical_resource_id = "the-update-physical-resource-id"
    elif request.is_delete_type():
        physical_resource_id = "the-delete-physical-resource-id"
    else:  # pragma: no cover
        raise ValueError(f"Unknown RequestType: {request.RequestType}")

    return Result(
        data=data,
        physical_resource_id=physical_resource_id,
        response_status=StatusEnum.SUCCESS,
        no_echo=False,
    )
